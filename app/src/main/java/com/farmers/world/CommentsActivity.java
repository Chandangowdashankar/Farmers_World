...
public class CommentsActivity extends AppCompatActivity {
		...
		edittext1.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				if (_charSeq.replace(" ", "").equals("")) {
					imageview1.setImageResource(R.drawable.ic_mic_black);
				}
				else {
					imageview1.setImageResource(R.drawable.ic_send_black);
				}
			}
			...
			public void onChildAdded(DataSnapshot _param1, String _param2) {
				...
				comment.addListenerForSingleValueEvent(new ValueEventListener() {
					...
					public void onDataChange(DataSnapshot _dataSnapshot) {
						...
						listview7.setAdapter(new Listview7Adapter(list_comment));
	...
	protected void onActivityResult(int _requestCode, int _resultCode, Intent _data) {
		...
		switch (_requestCode) {
			...
			if (_resultCode == RESULT_OK && null != _data) {
				...
				SketchwareUtil.showMessage(getApplicationContext(), "Google speech Recognizer Error!!");
			}
			break;
			default:
			break;
	...
	public class Listview7Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			_Shadow(20, 40, "#FFFFFF", linear1);
			if (list_comment.get((int)_position).get("uid").toString().equals(getIntent().getStringExtra("key"))) {
				textview3.setText(list_comment.get((int)_position).get("time").toString().concat(" - ".concat(list_comment.get((int)_position).get("date").toString())));
				if (list_comment.get((int)_position).containsKey("comment")) {
					textview2.setText(list_comment.get((int)_position).get("comment").toString());
				}
				if (list_comment.get((int)_position).containsKey("name")) {
					textview1.setText(list_comment.get((int)_position).get("name").toString());
				}
				if (list_comment.get((int)_position).get("verified").toString().equals("true")) {
					imageview1.setVisibility(View.VISIBLE);
				}
				else {
					if (list_comment.get((int)_position).get("verified").toString().equals("false")) {
			...
			if (list_comment.get((int)_position).get("id").toString().equals(FirebaseAuth.getInstance().getCurrentUser().getUid())) {
				textview1.setTextColor(0xFFFF5722);
			}
			else {
